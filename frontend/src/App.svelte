<script>
  ...
  import { siteSysop, siteSysopEmailAddress, siteInformation, siteGridSquare, siteHardware, siteSoftware, siteNote, siteSDRBaseFrequency, siteSDRBandwidth, siteCountry, siteChatEnabled } from '../site_information.json';
...
let bandwidth;
  ...
  function handleAutoAdjust(value) {
    ...
    // Added by NY4Q to track the waterfall settings //
  ...
  // This function was added by NY4Q to track band changes //
  ...
  function addBookmark() {
    ...
    const bookmark = {
      ...
      // This section was added by NY4Q to store more settings in the bookmark //
      volume: volume,
      squelch: squelch,
      squelchEnable: squelchEnable,
  ...
  function goToBookmark(bookmark) {
    ...
    // This next section was added by NY4Q to restore more //
    ...
    waterfall.setColormap(currentColormap);
    // End of additions added by NY4Q //
  ...
  // Added by NY4Q to allow the user to adjust the dynamic audio //
  ...
  <div class="h-screen overflow-hidden flex flex-col min-h-screen">
    <div class="w-full sm:h-screen overflow-y-scroll sm:w-1/2 xl:w-1/3 lg:w-1/4 sm:transition-all sm:ease-linear sm:duration-100" style="width:100%;">
      <div class="min-h-screen bg-custom-dark text-gray-200" style="padding-top: 5px;">
		...
		{siteSysop} e-mail:
		<a href="mailto:{siteSysopEmailAddress}?subject=WebSDR" style="color:rgba(0, 225, 255, 0.993)">{siteSysopEmailAddress}</a>&nbsp;&nbsp;Other PhantomSDR+ WebSDR servers at:&nbsp;
		<button class="glass-button text-white py-1 px-3 mb-2 lg:mb-0 rounded-lg text-xs sm:text-sm" style="color:rgba(0, 225, 255, 0.993)" on:click={() => window.open('https://sdr-list.xyz/', '_blank').focus()}>
                  <span class="icon">sdr-list.xyz</span>
                  </button>                
              </span>          
...
<!--
          <script>
            // JavaScript function to toggle the Additional Info menu
            function toggleMenu() {
              const menu = document.getElementById("collapsible-menu");
              const label = document.getElementById("menu-toggle-label");
...
<div class="grid grid-cols-4 gap-2">
        ...
        <button id="auto-adjust" class="retro-button text-sm text-white font-bold h-10 text-base rounded-md flex items-center justify-center border border-gray-600 shadow-inner transition-all duration-200 ease-in-out {autoAdjust === true ? 'bg-blue-600 pressed scale-95' : 'bg-gray-700 hover:bg-gray-600'}" on:click={() => handleAutoAdjust(autoAdjust)} title="Auto Adjust">                                                                         Auto Adjust
                          </button>
                  ...
                  <div class="bg-gray-800 p-6 rounded-lg max-w-lg w-full max-h-[80vh] flex flex-col">
                    ...
                    <div class="mb-6">
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <button class="glass-button text-white font-bold py-2 px-4 rounded-lg flex items-center" on:click={addBookmark}>
                          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <div class="mb-6">
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <button class="glass-button text-white font-bold py-2 px-4 rounded-lg flex items-center" on:click={handleLinkCopyClick}>
                          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
                      ...
                      {#each $bookmarks as bookmark, index}
                        <div class="glass-panel rounded-lg p-3 flex items-center justify-between mb-2">
                          ...
                          <div class="flex gap-2">
                            <button class="glass-button text-white font-bold py-1 px-3 rounded-lg flex items-center" on:click={() => goToBookmark(bookmark)}>
                              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" viewBox="0 0 20 20" fill="currentColor">
                                <path fill-rule="evenodd" d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z" clip-rule="evenodd" />
                              </svg>
                              Go
                            </button>
                            <button class="glass-button text-white font-bold py-1 px-3 rounded-lg flex items-center" on:click={() => copy(bookmark.link)}>
                              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" viewBox="0 0 20 20" fill="currentColor">
                                <path d="M8 3a1 1 0 011-1h2a1 1 0 110 2H9a1 1 0 01-1-1z" />
                                <path d="M6 3a2 2 0 00-2 2v11a2 2 0 002 2h8a2 2 0 002-2V5a2 2 0 00-2-2 3 3 0 01-3 3H9a3 3 0 01-3-3z" />
                              </svg>
                            ...
                            <button class="glass-button text-white font-bold py-1 px-3 rounded-lg flex items-center" on:click={() => deleteBookmark(index)}>
                              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" viewBox="0 0 20 20" fill="currentColor">
